...
interface ReviewStructDao {
    ...
    fun insertStructure(reviewStruct: review_struct)

    @Query("SELECT * FROM $REV_STRUTT WHERE id == :id")
    fun getSingleRevStruct(id: Int) : Struttura

    @Query("SELECT * FROM $REV_STRUTT Where review_id_struct == :id")
    fun getaLLRevOfAStructure(id:Int) : LiveData<List<Struttura>>

    @Query("SELECT * FROM $REV_STRUTT Where user_id == :id")
    fun getaLLRevDidByAnUser(id:Int) : LiveData<List<Struttura>>

    @Query("SELECT * FROM $REV_STRUTT Where id_campo == :id")
    fun getaLLRevOfAField(id:Int) : LiveData<List<Struttura>>
